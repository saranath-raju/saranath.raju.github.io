...
class BookModel {
    ...
    private func parseBookInfo(bookInfo: NSDictionary, book_received: Book, searchText: String) -> Book {
        ...
        if let publishedDate = bookInfo["publishedDate"] as? String {
            ...
            var date = dateFormatter.dateFromString(publishedDate)
    ...
    private func findPDFLink(accessInfo: NSDictionary, bookReceived: Book) -> Book {
        ...
        if let pdf = accessInfo["pdf"] as? NSDictionary {
            if let pdfavailable = pdf["isAvailable"] as? Bool {
                isPDFAvailable = pdfavailable
                if let pdfLink = accessInfo["webReaderLink"] as? String {
                    if isPDFAvailable {
                        book.pdfURL = pdfLink
                    }
                }
